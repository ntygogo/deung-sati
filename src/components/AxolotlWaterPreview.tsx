    ...
    const start = async () => {
      try {
        ...
        model.traverse(object => {
          ...
          const customGills=['earth','water','wind','fire','leaf','flower'].includes(appearance?.previewCollection ?? '');
          if(customGills)trimGillTopology(THREE,mesh,new Set(GILLS.flatMap(g=>[g.root,...g.mids,g.tip])));
          const positions = mesh.geometry.getAttribute('position');
          if(!customGills)mesh.geometry.setAttribute('axolotlGillCap',new THREE.BufferAttribute(new Float32Array(positions.count),1));
            ...
            material.onBeforeCompile = shader => {
              ...
              shader.fragmentShader = shader.fragmentShader
                ...
                .replace('#include <map_fragment>', `#include <map_fragment>
                  ...
                  diffuseColor.rgb = mix(diffuseColor.rgb,mix(axolotlFaceColor,axolotlFinColor,.72),axolotlCap);
