...
type Focus = { id: string; title: string; outcome: string; cue: string; action: string; obstacle: string; fallback: string };
type Entry = { id: string; at: string; focus: string; action: string; result: 'helpful' | 'little' | 'same' | 'harder' | 'notyet'; note: string };
type Journal = { focus: Focus | null; entries: Entry[] };
const choices = [
...
const read = (key: string): Journal => { const value = JSON.parse(localStorage.getItem(key) || '{"focus":null,"entries":[]}'); return { focus: value.focus || null, entries: Array.isArray(value.entries) ? value.entries : [] }; };
...
export function FutureSelf({ onBack, onChat }: { onBack: () => void; onChat: (text: string) => void }) {
  ...
  return <FutureSelfPage key={currentUser?.id || 'guest'} storageKey={`deung_sati_future_practice_v1_${currentUser?.id || 'guest'}`} onBack={onBack} onChat={onChat} />;
}
function FutureSelfPage({ storageKey, onBack, onChat }: { storageKey: string; onBack: () => void; onChat: (text: string) => void }) {
  const [journal, setJournal] = useState<Journal>(() => { try { return read(storageKey); } catch { return { focus: null, entries: [] }; } });
  const [editing, setEditing] = useState<Focus | null>(null);
  const [choosing, setChoosing] = useState(!journal.focus);
  ...
  const commit = (next: Journal) => {
    try { localStorage.setItem(storageKey, JSON.stringify(next)); setJournal(next); setError(''); return true; }
    catch { setError('ยังเก็บบันทึกในเบราว์เซอร์ไม่ได้ ข้อความยังอยู่ในหน้านี้ ลองอีกครั้งได้'); return false; }
  };
  const choose = (i: number) => { const item = choices[i]; setEditing({ id: crypto.randomUUID(), ...item }); setNotice(''); };
  const openReview = (value: Entry['result'] | '' = '') => { savedReview.current = false; setResult(value); setNote(''); setReviewing(true); setNotice(''); };
  const saveReview = () => {
    if (!focus || !result || savedReview.current) return;
    savedReview.current = true;
    const entry = { id: crypto.randomUUID(), at: new Date().toISOString(), focus: focus.title, action: currentAction, result, note: note.trim() };
  ...
  return <div className="screen scrollArea future-self">
    ...
    <main className="fs-content">
      ...
      {editing ? <form className="fs-plan" onSubmit={e => { e.preventDefault(); if (!editing.title.trim() || !editing.cue.trim() || !editing.action.trim()) { setError('เติมเรื่องที่อยากฝึก สถานการณ์ และก้าวที่จะลองก่อนนะ'); return; } if (commit({ ...journal, focus: editing })) { setEditing(null); setChoosing(false); setNotice('เลือกก้าวของเธอแล้ว ลองเมื่อเจอสถานการณ์จริง ไม่ต้องทำให้ได้ทันที'); } }}>
        ...
        <button type="submit" className="fs-primary">เก็บก้าวนี้ไว้ลอง <ArrowRight size={18} /></button>
        ...
        {reviewing && <section ref={reviewRef} className="fs-review" aria-label="ทบทวนหลังลอง"><div className="fs-section-head"><h2>{result === 'notyet' ? 'มีอะไรทำให้ยังไม่ได้ลอง?' : 'ก้าวนี้ช่วยเธอแค่ไหน?'}</h2><button className="fs-text-button" onClick={() => setReviewing(false)}>ไว้ก่อน</button></div><p className="fs-soft">ไม่มีคำตอบที่ต้องถูก และไม่ต้องรู้สึกดีขึ้นทุกครั้ง</p>
          ...
          <label>{result === 'notyet' ? 'เช่น ยังไม่มีจังหวะ ลืม หรือก้าวนี้ยังยากไป' : 'มีอะไรที่สังเกตเห็นไหม?'}<textarea maxLength={800} value={note} onChange={e => setNote(e.target.value)} placeholder="ยังอธิบายไม่ได้ก็เว้นไว้ได้" /></label><button className="fs-primary" disabled={!result} onClick={saveReview}>เก็บสิ่งที่เรียนรู้</button></section>}
      ...
      <p className="fs-storage">แผนและบันทึกหน้านี้เก็บในเบราว์เซอร์นี้<br />แยกตามบัญชีที่ใช้อยู่</p>
