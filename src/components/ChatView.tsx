...
export const ChatView: React.FC<ChatViewProps> = ({
  ...
  const streamAiResponse = async (history: ChatMessage[]) => {
    ...
    try {
      ...
      // Flush and process any remaining buffer after stream finishes
      ...
      // Finalize AI message with guaranteed complete text
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === aiMsgId
      ...
      // Check if conversation is rich enough to offer Loop Map (after 3+ turns in normal mode)
      if (history.length >= 3 && safetyState?.mode !== 'protect') {
