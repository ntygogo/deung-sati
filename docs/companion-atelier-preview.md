...
- App recognizes newly issued v1 snapshots whose stats metadata lacks `welcomedAt` and shows a dedicated welcome surface using the actual reserved 3D recipe.
- Embryo rests longer between gentle wriggles, with separately phased small leg kicks. Reveal waits for the renderer's ready callback, supports skipping and reduced motion, then offers naming and numbered PNG export.
- `POST /companion/welcome` requires authentication, validates 1–40 Unicode characters, locks the owned companion, updates the name and `stats_summary_json.welcomedAt` atomically, and leaves DNA unchanged. Replays from stale devices return the previously saved name. The normal app resumes only after a successful save; network failures retain the name form for retry.
- Legacy hatch celebration remains only for old non-v1 snapshots; its displayed reward now matches the server (50 XP / 25 Shells).
- Verified through real local HTTP handlers with an isolated SQLite database: two registrations, 20 practices each, distinct hatch reservations, concurrent same-owner retry, authentication and invalid-name rejection, naming, a second login/device reload, replay safety and identity preservation. This is not a live PostgreSQL test.
- Browser checks use mobile/touch-sized Chromium and desktop Chromium: embryo/reveal/naming, export, failed-save retry, persisted dismissal after reload, layout and WebGL errors. No physical iPhone/Android was available.
- Live Preview API testing remains blocked by Vercel's authentication layer: both connector fetch and a cookie-aware HTTP client reached the Vercel sign-in site rather than the application API. Deployment protection was not changed and no live test accounts/claims were created.
